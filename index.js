...
yargs.command({
    ...
    handler: function(argv) {
        fwrite.appendFileSync('./note.txt', "Title : " + argv.title)
        fwrite.appendFileSync('./note.txt', "\n")
        fwrite.appendFileSync('./note.txt', "Describe : " + argv.describe + "\n")
...
yargs.command({
    ...
    handler: function(argv) {
        fwrite.readFile('note.txt', { encoding: 'utf-8' }, function(err, data) {
            ...
            const searchKeyword = argv.title; // we are looking for a line, contains, key word 'user1' in the file
